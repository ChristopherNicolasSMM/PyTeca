...
_CHOICES_META   = get_choices_fields(Loan)  # lista de {field, label, order}


def _get_choices_filters(service: LoanService) -> list[FilterDef]:
    """
...
def list():
    ...
    if export in ("csv", "excel", "pdf"):
        ...
        if export == "pdf":
            return export_pdf(cfg_with_choices, all_result.items, visible_cols, title="Empréstimoss")
