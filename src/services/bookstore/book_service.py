...
# book_service_hooks.py é criado uma única vez pelo gerador
...
def _trash_value() -> str:
    ...
    """
    member = getattr(BookStatus, "TRASH", None)
...
class BookService:
    ...
    def trash(self, id: int) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao mover Book id=%s para lixeira: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
        return ServiceResult(success=True, data=obj)
