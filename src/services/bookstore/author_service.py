...
try:
    ...
    for _prop in Author.__mapper__.iterate_properties:
...
_TRASH_STATUS: str = getattr(AuthorStatus, "TRASH", type("_", (), {"value": "trash"})()).value     if hasattr(AuthorStatus, "TRASH") else "trash"
...
def _set_trash(obj) -> None:
    ...
    try:
        obj.status = AuthorStatus(_TRASH_STATUS)
...
class AuthorService:
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Author: %s", e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
        return ServiceResult(success=True, data=obj, code=201)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Author id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
        return ServiceResult(success=True, data=obj)
    ...
    def delete_permanent(self, id: int) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao excluir Author id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
